...
export function scoreCarrierForShipment(carrier, shipment, product) {
    const operatesAtDestination = carrier.operatesIn.includes(shipment.destination.country);
    const canHandleWeight = product.weightKg * shipment.quantity <= carrier.maxWeightKg;
    ...
    const score = (operatesAtDestination ? 20 : 0) +
...
export function findTopCarriers(shipments, // Recibe la lista de envíos a analizar.
topN) {
    if (topN <= 0) {
        return []; // Devuelve una lista vacía si no se solicita ningún resultado.
    }
    const carrierCounts = {}; // Inicializa los conteos por transportista.
    for (const shipment of shipments) {
    ...
    const rankedCarriers = Object.entries(carrierCounts).map(([carrier, count]) => ({ carrier, count }));
